...
// ─── RELACIONES BASE (reutilizadas en todos los queries) ──────────────────────
...
export class SalesReceiptRepository implements ISalesReceiptRepositoryPort {
  ...
  // findAll legacy — requerido por el port
  async findAll(filters?: {
    ...
    if (filters?.sedeId) {
      query.andWhere('receipt.id_sede_ref = :sedeId', {
    ...
    return raws
      .map((raw) => {
        try {
          return SalesReceiptMapper.toDomain(raw);
        } catch (e) {
          console.warn(
            `[SalesReceiptRepo] Registro inválido id=${raw.id_comprobante}: ${e.message}`,
          );
          return null;
        }
      })
      .filter((r): r is SalesReceipt => r !== null);
  }
  ...
  // findByIdWithFullRelations usa las mismas relaciones disponibles en el ORM
  ...
  async findAllWithRelations(
    ...
    // ── Conteo rápido (sin relaciones) ──
    const countQuery = this.receiptOrmRepository
      .createQueryBuilder('receipt')
      .where('receipt.numero > 0');

    if (filters.id_sede) {
      countQuery.andWhere('receipt.id_sede_ref = :id_sede', {
        id_sede: filters.id_sede,
      });
    }
    if (filters.estado) {
      countQuery.andWhere('receipt.estado = :estado', {
        estado: filters.estado,
      });
    }
    if (filters.fec_desde) {
      countQuery.andWhere('receipt.fec_emision >= :fec_desde', {
        fec_desde: filters.fec_desde,
      });
    }
    if (filters.fec_hasta) {
      const dateTo = new Date(filters.fec_hasta);
      dateTo.setHours(23, 59, 59, 999);
      countQuery.andWhere('receipt.fec_emision <= :fec_hasta', {
        fec_hasta: dateTo,
      });
    }
    if (filters.id_cliente) {
      countQuery.andWhere('receipt.id_cliente = :id_cliente', {
        id_cliente: filters.id_cliente,
      });
    }
    if (filters.id_tipo_comprobante) {
      countQuery.andWhere(
        'receipt.id_tipo_comprobante = :id_tipo_comprobante',
        { id_tipo_comprobante: filters.id_tipo_comprobante },
      );
    }
    if (filters.search) {
  ...
  async findCustomerPurchaseHistory(customerId: string): Promise<{
    ...
    // Primer comprobante del cliente (para extraer datos del cliente)
    ...
    // Últimas 10 compras con tipo de comprobante
