...
import { AccountReceivablePaymentOrmEntity } from '../../../entity/account-receivable-payment-orm.entity';
...
export class AccountReceivableRestController {
  ...
  @Get(':id/payments/:pagoId/voucher')
  async exportPaymentVoucher(
    @Param('id',     ParseIntPipe) id:     number,
    @Param('pagoId', ParseIntPipe) pagoId: number,
    @Res() res: Response,
  ) {
    return await this.queryService.exportPaymentVoucher(id, pagoId, res);
  }
  
  // ── Pagos de una cuenta ───────────────────────────────────────────
  /**
   * GET /account-receivables/:id/payments
   * Devuelve el historial de abonos registrados para una cuenta.
   */
  @Get(':id/payments')
  async getPayments(@Param('id', ParseIntPipe) id: number) {
    ...
    return pagos.map((p) => ({
      id:          p.id,
      fec_pago:    p.fecPago,
      metodo_pago: p.paymentType?.descripcion ?? String(p.paymentTypeId),
